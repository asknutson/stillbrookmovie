...
if (!function_exists('xpand_blog_custom_excerpt_length')):
    function xpand_blog_custom_excerpt_length($length)
    {
        if (is_admin())
        {
            return $length;
        }
        $ext_length = get_theme_mod('xpand_blog_excerpt_length', '55', 'xpand-blog');
        if (!empty($ext_length))
        {
            return $ext_length;
        }
...
add_filter('excerpt_length', 'xpand_blog_custom_excerpt_length', 999);

function xpand_blog_sanitize_select($input, $setting)
{
...
function xpand_blog_sanitize_checkbox($checked)
{
    return ((isset($checked) && true === $checked) ? true : false);
}

if (!class_exists('Xpand_Blog_Repeater_Setting'))
{

    /**
     * Repeater Settings.
     */
    class Xpand_Blog_Repeater_Setting extends WP_Customize_Setting
    {
        ...
        public function __construct($manager, $id, $args = array())
        {
            ...
            add_filter("customize_sanitize_{$this->id}", array(
                $this,
...
if (!class_exists('Xpand_Blog_Control_Repeater'))
{
    /**
     * Repeater control
     */
    class Xpand_Blog_Control_Repeater extends WP_Customize_Control
    {
         ...
         * @var string
         */
        public $type = 'xpand-blog-repeater';
        ...
        public function __construct($manager, $id, $args = array())
        {
            ...
            $this->row_label = array(
                'type' => 'text',
                'value' => esc_attr__('row', 'xpand-blog') ,
                'field' => false,
            );
            ...
            {
                $this->button_label = sprintf(esc_attr__('Add new %s', 'xpand-blog') , $this->row_label['value']);
            }
        ...
        public function to_json()
        {
            ...
            $this->json['default'] = (isset($this->default)) ? $this->default : $this
                ->setting->default;
        ...
        public function enqueue()
        {
            ...
            if (is_array($this->fields) && !empty($this->fields))
            {
                foreach ($this->fields as $field)
                {
                    if (isset($field['type']))
                    {
                        ...
                        {
                            wp_enqueue_script('all', get_template_directory_uri() . '/assets/js/all.js', array(
                                'jquery'
                            ) , '5.6.3', true);
                            wp_enqueue_script('v4-shims', get_template_directory_uri() . '/assets/js/v4-shims.js', array(
                                'jquery',
                                'all'
                            ) , '5.6.3', true);
                        }
            ...
            wp_enqueue_script('xpand-blog-repeater', get_template_directory_uri() . '/assets/js/repeater.js', array(
                'jquery',
                'jquery-ui-core',
                'jquery-ui-sortable'
            ) , false, true);
            wp_enqueue_style('xpand-blog-repeater', get_template_directory_uri() . '/assets/css/repeater.css', null);
        }
...
?>
            ...
            <?php if (isset($this->choices['limit'])): ?>
                <p class="limit"><?php printf(esc_html__('Limit: %s rows', 'xpand-blog') , esc_html($this->choices['limit'])); ?></p>
            <?php
            endif; ?>
            ...
            <script type="text/html" class="customize-control-repeater-content">
                ...
                <li class="repeater-row minimized" data-row="{{{ index }}}">
                    ...
                    <div class="repeater-row-content">
                        <# _.each( data, function( field, i ) { #>
                            ...
                            <div class="repeater-field repeater-field-{{{ field.type }}}">
                                ...
                                <# } else if ( 'color' === field.type ) { #>
                                    ...
                                    <label>
                                        ...
                                        <# if ( field.description ) { #>
                                            <span class="description customize-control-description">{{{ field.description }}}</span>
                                        <# } #>
                                        <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__('Hex Value', 'xpand-blog'); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
                                ...
                                <# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
                                    ...
                                    <figure class="xpand-blog-image-attachment" data-placeholder="<?php esc_attr_e('No Image Selected', 'xpand-blog'); ?>" >
                                        <# if ( field.default ) { #>
                                            <# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
                                            <img src="{{{ defaultImageURL }}}">
                                        <# } else { #>
                                            <?php esc_attr_e('No Image Selected', 'xpand-blog'); ?>
                                        <# } #>
                                    </figure>
    
                                    <div class="actions">
                                        <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e('Remove', 'xpand-blog'); ?></button>
                                        <button type="button" class="button upload-button" data-label=" <?php esc_attr_e('Add Image', 'xpand-blog'); ?>" data-alt-label="<?php esc_attr_e('Change Image', 'xpand-blog'); ?>" >
                                            <# if ( field.default ) { #>
                                                <?php esc_attr_e('Change Image', 'xpand-blog'); ?>
                                            <# } else { #>
                                                <?php esc_attr_e('Add Image', 'xpand-blog'); ?>
                                            <# } #>
                                        </button>
                                        <# if ( field.default.id ) { #>
                                ...
                                <# } else if ( field.type === 'upload' ) { #>
                                    ...
                                    <figure class="xpand-blog-file-attachment" data-placeholder="<?php esc_attr_e('No File Selected', 'xpand-blog'); ?>" >
                                        <# if ( field.default ) { #>
                                        ...
                                        <# } else { #>
                                            <?php esc_attr_e('No File Selected', 'xpand-blog'); ?>
                                        <# } #>
                                    </figure>
    
                                    <div class="actions">
                                        <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
                                        <button type="button" class="button upload-button" data-label="<?php esc_attr_e('Add File', 'xpand-blog'); ?>" data-alt-label="<?php esc_attr_e('Change File', 'xpand-blog'); ?>" >
                                            <# if ( field.default ) { #>
                                                <?php esc_attr_e('Change File', 'xpand-blog'); ?>
                                            <# } else { #>
                                                <?php esc_attr_e('Add File', 'xpand-blog'); ?>
                                            <# } #>
                                        </button>
                                        <# if ( field.default.id ) { #>
                                            <input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
                        ...
                        <# }); #>
                        <button type="button" class="button-link repeater-row-remove"><?php esc_html_e('Remove', 'xpand-blog'); ?></button>
                    </div>
                </li>
            </script>
            <?php

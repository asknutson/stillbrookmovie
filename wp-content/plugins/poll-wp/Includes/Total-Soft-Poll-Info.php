<?php
	...
	{
		$Total_Soft_Poll_Info_Quest = esc_html($Total_Soft_Poll_Questions[0]->id);
	}
	...
	<table class="Total_Soft_Poll_Info_Table">
		...
		<?php for( $i = 0; $i < count($Total_Soft_Poll_Info); $i++) { ?>
			<tr>
				...
				<td> <?php echo esc_html($Total_Soft_Poll_Info[$i]->Data); ?> </td>
			<td> Unknown </td>
				<td> Unknown (UN) </td>
				<td> Unknown </td>
				<td> Unknown </td>
				<td> <img src="<?php echo plugins_url('../Images/Flags/UN.png',__FILE__);?>" style="vertical-align: middle;"> </td>
				<td> <?php echo esc_html($Total_Soft_Poll_Info[$i]->Votes); ?> </td>
			</tr>
		<?php } ?>
	</table>
</form>
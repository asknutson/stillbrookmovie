...
<form method="POST" oninput="" style="position: relative;">
	...
	<div class="TS_Poll_Results_Table_O1">
	<table class="Total_Soft_Poll_Results_Table">
		<div class="TS_Poll_Results_Table_O"></div>
		<tr>
			<td colspan="4">
				<select onchange="this.form.submit()" name="Total_Soft_Poll_Results_Quest" class="Total_Soft_Poll_Results_Quest">
					<?php for( $i = 0; $i < count($Total_Soft_Poll_Questions); $i++ ){ ?>
						<?php if($Total_Soft_Poll_Questions[$i]->id == $Total_Soft_Poll_Results_Quest){ ?>
							<option selected value="<?php echo esc_attr($Total_Soft_Poll_Questions[$i]->id);?>"><?php echo html_entity_decode(esc_html($Total_Soft_Poll_Questions[$i]->TotalSoftPoll_Question));?></option>
						<?php } else { ?>
							<option value="<?php echo esc_attr($Total_Soft_Poll_Questions[$i]->id);?>"><?php echo html_entity_decode(esc_html($Total_Soft_Poll_Questions[$i]->TotalSoftPoll_Question));?></option>
						<?php }?>
					<?php }?>
				</select>
			</td>
		...
		<tr>
			...
			<td>Answer</td>
			<td>Media</td>
			<td>Votes <img src="<?php echo plugins_url('../Images/pro-icon.png',__FILE__);?>"></td>
		</tr>
		<?php for( $i = 0; $i < count($Total_Soft_Poll_Results); $i++) { ?>
			<tr>
				<td> <?php echo esc_html($i+1); ?> </td>
				<td> <?php echo esc_html($Total_Soft_Poll_Answers[$i]->TotalSoftPoll_Ans); ?> </td>
				<td> <img class="TotalSoftPollAnsImage" src="<?php echo esc_url($Total_Soft_Poll_Answers[$i]->TotalSoftPoll_Ans_Im); ?>"> </td>
				<td> <input type="text" name="Total_Soft_Poll_Results_<?php echo esc_html($i+1); ?>" value="<?php echo esc_attr($Total_Soft_Poll_Results[$i]->Poll_A_Votes); ?>"> </td>
			</tr> 
		<?php } ?>
	</table>
	</div>

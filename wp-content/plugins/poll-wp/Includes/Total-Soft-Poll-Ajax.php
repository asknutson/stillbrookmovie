<?php
	//ajax only for admin
	add_action('init','admin_ajax');
	function admin_ajax(){
		if(current_user_can('administrator')){
			add_action( 'wp_ajax_TotalSoftPoll_Clone', 'T_S_P_Clone_Callback' );		
			add_action( 'wp_ajax_TotalSoftPoll_Del', 'T_S_P_Del_Callback' );		
			add_action( 'wp_ajax_TotalSoftPoll_Edit', 'T_S_P_Edit_Callback' );		
			add_action( 'wp_ajax_TotalSoftPoll_Edit_Q_M', 'T_S_P_Edit_Q_M_Callback' );		
			add_action( 'wp_ajax_TotalSoftPoll_Edit_Ans', 'T_S_P_Edit_Ans_Callback' );		
			add_action( 'wp_ajax_TotalSoftPoll_Theme_Clone', 'T_S_P_Theme_Clone_Callback' );		
			add_action( 'wp_ajax_TotalSoftPoll_Theme_Edit', 'T_S_P_Theme_Edit_Callback' );		
			add_action( 'wp_ajax_TotalSoftPoll_Theme_Edit1', 'T_S_P_Theme_Edit1_Callback' );		
			add_action( 'wp_ajax_TotalSoftPoll_Clone_Set', 'T_S_P_Clone_Set_Callback' );		
			add_action( 'wp_ajax_TotalSoftPoll_Edit_Set', 'T_S_P_Edit_Set_Callback' );		
			add_action( 'wp_ajax_TotalSoftPoll_Del_Set', 'T_S_P_Del_Set_Callback' );		
		}
	}
	add_action( 'wp_ajax_TSoftPoll_Vimeo_Video_Image', 'T_S_P_Vimeo_Video_Image_Callback' );	
	add_action( 'wp_ajax_TotalSoftPoll_1_Vote', 'T_S_P_1_Vote_Callback' );	
	add_action( 'wp_ajax_nopriv_TotalSoftPoll_1_Vote', 'T_S_P_1_Vote_Callback' );
	add_action( 'wp_ajax_TotalSoftPoll_1_Results', 'T_S_P_1_Results_Callback' );	
	add_action( 'wp_ajax_TS_PTable_New_MTable_DisMiss_Poll', 'T_S_P_Table_New_MTable_DisMiss_Callback_Poll' );	
	add_action( 'wp_ajax_TS_Poll_Question_DisMiss', 'T_S_P_Question_DisMiss_Callback' );	
	add_action( 'wp_ajax_Total_Soft_Poll_Prev', 'T_S_P_Prev_Callback' );	

	function T_S_P_Vimeo_Video_Image_Callback()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			wp_die("Error");
		}
			$GET_Video_Video_Src = sanitize_text_field($_POST['foobar']);
			$TSoft_Poll_Image_Src = explode('video/',$GET_Video_Video_Src);
			$TSoft_Poll_Image_Src_Real = wp_remote_get("http://vimeo.com/api/v2/video/$TSoft_Poll_Image_Src[1].php");
			$TSoft_Poll_Image_Src_Real = unserialize(wp_remote_retrieve_body( $TSoft_Poll_Image_Src_Real ));
			wp_send_json($TSoft_Poll_Image_Src_Real[0]);
			wp_die();
	}


	function T_S_P_Clone_Callback()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			wp_die("Error");
		}
	...
	function T_S_P_Edit_Callback()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			...
			wp_send_json($Total_Soft_Poll_Manager);
			wp_die();
	}


	function T_S_P_Edit_Q_M_Callback()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			wp_die("Error");
		}
	...
	function T_S_P_Edit_Ans_Callback()
	{
		...
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			...
			wp_send_json($TotalSoftPoll_Edit_Answers);
			wp_die();

	}
	// Theme menu

	function T_S_P_Theme_Clone_Callback()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			wp_die("Error");
		}
			...
			if($TotalSoft_Poll_Theme[0]->TotalSoft_Poll_TType == 'Standart Poll' || $TotalSoft_Poll_Theme[0]->TotalSoft_Poll_TType == 'Standard Poll')
			{
			...
			else if($TotalSoft_Poll_Theme[0]->TotalSoft_Poll_TType == 'Standart Without Button' || $TotalSoft_Poll_Theme[0]->TotalSoft_Poll_TType == 'Standard Without Button')
			{
	...
	function T_S_P_Theme_Edit_Callback()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			wp_die("Error");
		}
			...
			wp_send_json($Total_Soft_Poll_Theme);
			wp_die();
	}


	function T_S_P_Theme_Edit1_Callback()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			wp_die("Error");
		}
	...
	// Voting empty($_POST['nonce']) ||

	function T_S_P_1_Vote_Callback()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			wp_die("Error");
		}
			...
			$Total_Soft_IP_res = wp_remote_get('http://www.geoplugin.net/php.gp?ip='.$Total_Soft_IP_Address);
			$Total_Soft_IP_Address_Info = unserialize(wp_remote_retrieve_body( $Total_Soft_IP_res ));
			if(empty($Total_Soft_IP_Address_Info['geoplugin_countryCode']))
			{
				...
				$Total_Soft_Poll_Answer = 'Unknown';
			}

			$wpdb->query($wpdb->prepare("INSERT INTO $table_name7 (id, Poll_ID, IPAddress, City, Region, CountryCode, CountryName, CountryFlag, Votes, Data) VALUES (%d, %s, %s, %s, %s, %s, %s, %s, %s, %s)", '', $Total_Soft_Poll_Question_ID, $Total_Soft_IP_Address, $Total_Soft_IP_Address_Info['geoplugin_city'], $Total_Soft_IP_Address_Info['geoplugin_region'], $Total_Soft_IP_Address_Info['geoplugin_countryCode'], $Total_Soft_IP_Address_Info['geoplugin_countryName'], $Total_Soft_IP_Address_Info['geoplugin_countryCode'], $Total_Soft_Poll_Answer, date("Y.m.d h:i:sa")));
			...
			wp_send_json($Total_Soft_Poll_Results);
			wp_die();
	}
	// Results

	function T_S_P_1_Results_Callback()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			wp_die("Error");
		}
	...
	function T_S_P_Clone_Set_Callback()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			wp_die("Error");
		}
	...
	function T_S_P_Edit_Set_Callback()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			wp_die("Error");
		}
			...
			wp_send_json($TotalSoftPollSetManager);
			wp_die();
	}


	function T_S_P_Del_Set_Callback()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			wp_die("Error");
		}
	...
	function T_S_P_Table_New_MTable_DisMiss_Callback_Poll()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			wp_die("Error");
		}
	...
	function T_S_P_Question_DisMiss_Callback()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			wp_die("Error");
		}
	...
	function T_S_P_Prev_Callback()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			wp_die("Error");
		}
			$Poll_TSet_Non =  json_decode(stripcslashes($_POST['foobar']));
			$Poll_TSet = sanitize_text_field($Poll_TSet_Non);
			global $wpdb;
			...
			if($Poll_TSet[1] == 'Standart Poll' || $Poll_TSet[1] == 'Standard Poll')
			{
			...
			else if($Poll_TSet[1] == 'Standart Without Button' || $Poll_TSet[1] == 'Standard Without Button')
			{

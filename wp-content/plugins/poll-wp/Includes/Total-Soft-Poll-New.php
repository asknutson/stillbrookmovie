...
if ( $_SERVER["REQUEST_METHOD"] == "POST" ) {
	if ( check_admin_referer( 'edit-menu_', 'TS_Poll_Nonce' ) ) {
		$TotalSoftPoll_Question = str_replace( "\&", "&", sanitize_text_field( wp_unslash( $_POST['TotalSoftPoll_Question'] ) ) );
		...
		$TotalSoftPoll_Old_Ans    = array( 0 );
		$TotalSoftPoll_Ans_Im     = array();
		$TotalSoftPoll_Old_Ans_Im = array( 0 );
		$TotalSoftPoll_Ans_Vd     = array();
		$TotalSoftPoll_Old_Ans_Vd = array( 0 );
		$TotalSoftPoll_Ans_Cl     = array();
		$TotalSoftPoll_Ans_Old_Cl = array( 0 );

		function getUniqueKeyForAnswer( $answer ) {
			return implode( '_', $answer );
		}

		for ( $i = 1; $i <= $TotalSoftPollHidNum; $i ++ ) {
			$TotalSoftPoll_Ans[ $i ]    = str_replace( "\&", "&", sanitize_text_field( wp_unslash( $_POST[ 'TotalSoftPoll_Ans_' . $i ] ) ) );
		...
		foreach ( $_POST as $key => $value ) {
			if ( strpos( $key, 'TotalSoftPoll_Old_Ans_' ) !== false ) {
				$TotalSoftPoll_Old_Ans[] = str_replace( "\&", "&", sanitize_text_field( wp_unslash( $value ) ) );
			}
			if ( strpos( $key, 'TotalSoftPoll_Old_Col_' ) !== false ) {
				$TotalSoftPoll_Ans_Old_Cl[] = str_replace( "\&", "&", sanitize_text_field( wp_unslash( $value ) ) );
			}
			if ( strpos( $key, 'TotalSoftPoll_Old_Im_' ) !== false ) {
				$TotalSoftPoll_Old_Ans_Im[] = str_replace( "\&", "&", sanitize_text_field( wp_unslash( $value ) ) );
			}
			if ( strpos( $key, 'TotalSoftPoll_Old_Vd_' ) !== false ) {
				$TotalSoftPoll_Old_Ans_Vd[] = str_replace( "\&", "&", sanitize_text_field( wp_unslash( $value ) ) );
			}
		}
		...
		if ( isset( $_POST['Total_Soft_Poll_Save'] ) ) {
			...
			for ( $i = 0; $i < count( $Total_Soft_Poll_Answers ); $i ++ ) {
				...
				$array[ getUniqueKeyForAnswer(
					[
						$Total_Soft_Poll_Answers[ $i ]->Question_ID,
						$Total_Soft_Poll_Answers[ $i ]->TotalSoftPoll_Ans,
						$Total_Soft_Poll_Answers[ $i ]->TotalSoftPoll_Ans_Im,
						$Total_Soft_Poll_Answers[ $i ]->TotalSoftPoll_Ans_Vd,
						$Total_Soft_Poll_Answers[ $i ]->TotalSoftPoll_Ans_Cl,
					]
				) ] = $Total_Soft_Poll_Votes[ $i ]->Poll_A_Votes;
			}
			...
			for ( $i = 1; $i <= $TotalSoftPollHidNum; $i ++ ) {
				...
				$vote = $array[ getUniqueKeyForAnswer( [
					$Total_SoftPoll_Update,
					$TotalSoftPoll_Ans[ $i ],
					$TotalSoftPoll_Ans_Im[ $i ],
					$TotalSoftPoll_Ans_Vd[ $i ],
					$TotalSoftPoll_Ans_Cl[ $i ]
				] ) ];

				if ( ! $vote ) {
					$vote = $array[ getUniqueKeyForAnswer( [
						$Total_SoftPoll_Update,
						$TotalSoftPoll_Old_Ans[ $i ],
						$TotalSoftPoll_Old_Ans_Im[ $i ],
						$TotalSoftPoll_Old_Ans_Vd[ $i ],
						$TotalSoftPoll_Ans_Old_Cl[ $i ]
					] ) ];
				}
...
<form method="POST" enctype="multipart/form-data">
    ...
    <div class="Total_Soft_Poll_AMD">
        ...
        <div class="Total_Soft_Poll_AMD2">
            <i class="Total_Soft_Poll_Help totalsoft totalsoft-question-circle-o"
               title="Click for creating new Poll."></i>
            <span class="Total_Soft_Poll_AMD2_But"
                  onclick="Total_Soft_Poll_AMD2_But1(<?php echo esc_attr($TotalSoftPollShortID[0]->Poll_ID + 1); ?>)">
				Create Poll
			</span>
        </div>
        <div class="Total_Soft_Poll_AMD3">
            <i class="Total_Soft_Poll_Help totalsoft totalsoft-question-circle-o"
               title="Click for refreshing the page."></i>
            <span class="Total_Soft_Poll_AMD2_But" onclick="TotalSoftPoll_Reload()">
				Cancel
			</span>
            <i class="Total_Soft_Poll_Save Total_Soft_Poll_Help totalsoft totalsoft-question-circle-o"
               title="Click for saving the Poll."></i>
            <button type="submit" class="Total_Soft_Poll_Save Total_Soft_Poll_AMD2_But" name="Total_Soft_Poll_Save">
                Save
            </button>
            <i class="Total_Soft_Poll_Update Total_Soft_Poll_Help totalsoft totalsoft-question-circle-o"
               title="Click for updating the Poll."></i>
    ...
    <table class="Total_Soft_Poll_AMOTable">
					<?php for ( $i = 0; $i < count( $TotalSoftPollCount ); $i ++ ) {
						...
						?>
         <tr id="Total_Soft_Poll_AMOTable_tr_<?php echo esc_html($TotalSoftPollCount[ $i ]->id); ?>">
             <td><?php echo esc_html($i + 1); ?></td>
             <td><?php echo esc_html($TotalSoftPollCount[ $i ]->TotalSoftPoll_Question); ?></td>
             <td><?php echo esc_html($TotalSoftPollCount[ $i ]->TotalSoftPoll_Ans_C); ?></td>
             <td><?php echo esc_html($TotalSoftPollThemName[0]->TotalSoft_Poll_TTitle); ?></td>
             <td><i class="totalsoft totalsoft-file-text"
                    onclick="TotalSoftPoll_Clone(<?php echo esc_attr($TotalSoftPollCount[ $i ]->id); ?>)"></i></td>
             <td><i class="totalsoft totalsoft-pencil"
                    onclick="TotalSoftPoll_Edit(<?php echo esc_attr($TotalSoftPollCount[ $i ]->id); ?>)"></i></td>
             <td>
                 <i class="totalsoft totalsoft-trash"
                    onclick="TotalSoftPoll_Del(<?php echo esc_attr($TotalSoftPollCount[ $i ]->id); ?>)"></i>
                 <span class="Total_Soft_Poll_Del_Span">
						<i class="Total_Soft_Poll_Del_Span_Yes totalsoft totalsoft-check"
                           onclick="TotalSoftPoll_Del_Yes(<?php echo esc_attr($TotalSoftPollCount[ $i ]->id); ?>)"></i>
						<i class="Total_Soft_Poll_Del_Span_No totalsoft totalsoft-times"
                           onclick="TotalSoftPoll_Del_No(<?php echo esc_attr($TotalSoftPollCount[ $i ]->id); ?>)"></i>
					</span>
             </td>
         </tr>
					<?php } ?>
    ...
    <table class="Total_Soft_Poll_AMShortTable">
        ...
        <tr style="text-align:center">
            <td>
                ...
                <i class="Total_Soft_Poll_Help3 totalsoft totalsoft-files-o" title="Click to Copy."
                   onclick="Copy_Shortcode_Poll('Total_Soft_Poll_ID')"></i>
            </td>
            <td>
                <span id="Total_Soft_Poll_TID"></span>
                <i class="Total_Soft_Poll_Help3 totalsoft totalsoft-files-o" title="Click to Copy."
                   onclick="Copy_Shortcode_Poll('Total_Soft_Poll_TID')"></i>
            </td>
        </tr>
    </table>
            ...
            <table class="TS_Poll_Add_Answer_Table">
                ...
                <tr>
                    <td>
                        <div id="wp-content-media-buttons" class="wp-media-buttons">
                            <a href="#" class="button insert-media add_media"
                               style="border:1px solid #009491; color:#009491; background-color:#f4f4f4"
                               data-editor="TotalSoftPoll_Video_1" title="Add Video" id="TotalSoftPoll_Video"
                               onclick="TotalSoftPoll_Video_Clicked('TotalSoftPoll')">
                    ...
                    <td style="position: relative;">
                        ...
                        <i class="TSPDUFS totalsoft totalsoft-times" aria-hidden="true"
                           onclick="TSPDUFS_Cl('TotalSoftPoll_Video_2')"></i>
                    </td>
                </tr>
                <tr>
                    <td>
                        <div id="wp-content-media-buttons" class="wp-media-buttons">
                            <a href="#" class="button insert-media add_media"
                               style="border:1px solid #009491; color:#009491; background-color:#f4f4f4"
                               data-editor="TotalSoftPoll_Image_1" title="Add Image" id="TotalSoftPoll_Image"
                               onclick="TotalSoftPoll_Image_Clicked('TotalSoftPoll')">
                    ...
                    <td style="position: relative;">
                        ...
                        <i class="TSPDUFS totalsoft totalsoft-times" aria-hidden="true"
                           onclick="TSPDUFS_Cl('TotalSoftPoll_Image_2')"></i>
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
                        <span class="Total_Soft_Poll_AMD2_But" onclick="TS_Poll_Add_Answer_Button_Close()">Close</span>
                        <span class="Total_Soft_Poll_AMD2_But" id="Total_Soft_Poll_UpdAns"
                              onclick="Total_Soft_Poll_Update_Ans()">Update Answer</span>
                        <span class="Total_Soft_Poll_AMD2_But" id="Total_Soft_Poll_SavAns"
                              onclick="Total_Soft_Poll_Save_Ans()">Save Answer</span>
    ...
    <div class="TS_Poll_AM_Table_Div">
        ...
        <table class="Total_Soft_Poll_Add_MTable">
            <tr>
                <td>Question</td>
                <td>
                    <input type="text" class="Total_Soft_Poll_Select" name="TotalSoftPoll_Question"
                           id="TotalSoftPoll_Question" required placeholder=" * Required">
                </td>
            </tr>
            <tr>
                <td>Settings</td>
                <td>
                    <select class="Total_Soft_Poll_Select" id="TotalSoft_Poll_Gen_Set" name="TotalSoft_Poll_Gen_Set">
                         ...
                         <option value="<?php echo esc_attr($TotalSoftPollSetCount[ $i ]->id); ?>"><?php echo esc_html($TotalSoftPollSetCount[ $i ]->TotalSoft_Poll_SetTitle); ?></option>
																					<?php } ?>
                    </select>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <select class="Total_Soft_Poll_Select" id="TotalSoftPoll_Theme" name="TotalSoftPoll_Theme"
                            onchange="TotalSoftPoll_Type_Ch()">
																					<?php for ( $i = 0; $i < count( $TotalSoftPollThemes ); $i ++ ) { ?>
                         <option id="TotalSoftPoll_Theme_<?php echo esc_attr($TotalSoftPollThemes[ $i ]->id); ?>"
                                 value="<?php echo esc_attr($TotalSoftPollThemes[ $i ]->id); ?>"
                                 rel="<?php echo esc_attr($TotalSoftPollThemes[ $i ]->TotalSoft_Poll_TType); ?>"><?php echo esc_html($TotalSoftPollThemes[ $i ]->TotalSoft_Poll_TTitle); ?></option>
																					<?php } ?>
                    </select>
                </td>
            </tr>
            <tr>
                <td>
                    <div id="wp-content-media-buttons" class="wp-media-buttons">
                        <a href="#" class="button insert-media add_media"
                           style="border:1px solid #009491; color:#009491; background-color:#f4f4f4"
                           data-editor="TotalSoftPollQ_Video_1" title="Add Video" id="TotalSoftPollQ_Video"
                           onclick="TotalSoftPoll_Video_Clicked('TotalSoftPollQ')">
                            <span class="wp-media-buttons-icon"></span>Add Video
                        </a>
                    </div>
                ...
                <td>
                    <input type="text" class="Total_Soft_Poll_Select" id="TotalSoftPollQ_Video_2"
                           name="TotalSoftPollQ_Video_2" readonly>
                    <i class="TSPDUFS totalsoft totalsoft-times" aria-hidden="true"
                       onclick="TSPDUFS_Cl('TotalSoftPollQ_Video_2')"></i>
                </td>
            </tr>
            <tr>
                <td>
                    <div id="wp-content-media-buttons" class="wp-media-buttons">
                        <a href="#" class="button insert-media add_media"
                           style="border:1px solid #009491; color:#009491; background-color:#f4f4f4"
                           data-editor="TotalSoftPollQ_Image_1" title="Add Image" id="TotalSoftPollQ_Image"
                           onclick="TotalSoftPoll_Image_Clicked('TotalSoftPollQ')">
                            <span class="wp-media-buttons-icon"></span>Add Image
                        </a>
                    </div>
                ...
                <td>
                    <input type="text" class="Total_Soft_Poll_Select" id="TotalSoftPollQ_Image_2"
                           name="TotalSoftPollQ_Image_2" readonly>
                    <i class="TSPDUFS totalsoft totalsoft-times" aria-hidden="true"
                       onclick="TSPDUFS_Cl('TotalSoftPollQ_Image_2')"></i>
                </td>
            </tr>
            <tr>
                <td colspan="2">
                    <div class="TS_Poll_AM_ANV">
						<span class="TS_Poll_AM_ANV_Span1" onclick="TS_Poll_Add_Answer_Button()">
							<span class="TS_Poll_AM_ANV_Span2">
								<i class="TS_Poll_AM_ANV_Icon totalsoft totalsoft-plus-circle"
                                   style="margin-right: 5px;"></i>
								Add Answer
							</span>
						</span>
                    </div>
                    <input type="text" style="display:none;" id="TotalSoftPollHidNum" name="TotalSoftPollHidNum"
                           value="0">
                </td>
            </tr>
        </table>
